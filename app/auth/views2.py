...
from auth.rate_limit import rate_limit, verify_recaptcha, CAPTCHA_SITE_KEY
...
def register():
    ...
    return render_template("auth/register.html", recaptcha_site_key=CAPTCHA_SITE_KEY)


@auth.route("/create-user", methods=["POST"])
@rate_limit("registration")  # Add rate limiting decorator
def create_user():
    ...
    # Debug information
    print(f"Creating user: {name}, {username}, {email}")
...
def add_credential():
    ...
    try:
        ...
        print(f"InvalidRegistrationResponse: {str(e)}")
        abort(make_response('{"verified": false}', 400))
    except Exception as e:
        print(f"Unexpected error: {str(e)}")
        abort(make_response('{"verified": false, "error": "' + str(e) + '"}', 400))
